...
//	private String role_api_schema;
	...
	public List<RoleBean> findAllRoles() {
		try {
			...
			String iam_role_api_url = appUtils.iam_server_api_base_url + appUtils.role_api_url;
			...
			if (HttpStatus.OK.value() == apiResponse.getStatusCode().value()) {
				...
				resourcesList = appUtils.parseRoleResponse(responseBody);
	...
	public List<RoleBean> createRole(RoleBean roleBean) {
		try {
			...
			List<RoleBean> resourcesList = null;
			String iam_role_api_url = appUtils.iam_server_api_base_url + appUtils.role_api_url;
			roleBean.setSchemas(new String[] { appUtils.role_api_schema });
			String rolePayload = objectMapper.writeValueAsString(roleBean);

			HttpEntity<String> requestBody = appUtils.prepareRequestBody(rolePayload);
